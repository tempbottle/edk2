  ...
  )
{
  ...
  // FIXME: Check endianness of ELF data (hdr->e_ident[EI_DATA])?
  ...
  )
{
  ...
  if (ProgramHdr->p_filesz != 0) {
    ...
    CopyMem (MemSegment, FileSegment, (UINTN)ProgramHdr->p_filesz);
  }

  ExtraZeroes = (UINT8 *)((UINTN)MemSegment + ProgramHdr->p_filesz);
  ExtraZeroesCount = ProgramHdr->p_memsz - ProgramHdr->p_filesz;
  DEBUG ((EFI_D_INFO, "Completing segment with %d zero bytes.\n", ExtraZeroesCount));
  for (Index = 0; Index < ExtraZeroesCount; ++Index) {
    ExtraZeroes[Index] = 0;
  }
  ...
  *ImageSize = 0;
  ...
  for (Index = 0; Index < ElfHdr->e_phnum; ++Index) {
    ...
    if (ProgramHdrPtr->p_type == PT_LOAD) {
      Status = ElfLoadSegment (ElfImage, (VOID *)ProgramHdrPtr);
      if (EFI_ERROR (Status)) {
